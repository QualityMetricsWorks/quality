...
export function metricsForRuns(runs){
 ...
 const copq=events.reduce((s,e)=>s+copqForEvent(e),0);const pieceCost=runs.reduce((sum,r)=>sum+Number(r.produced||0)*Number(getPart(r.partId)?.costPerPiece||0),0);const scrapCost=events.filter(e=>e.disposition==='scrap').reduce((sum,e)=>sum+Number(e.quantity||0)*Number((getPart(getRun(e.runId)?.partId)?.scrapCostPerPiece>0?getPart(getRun(e.runId)?.partId).scrapCostPerPiece:getPart(getRun(e.runId)?.partId)?.costPerPiece)||0),0);const copqPercent=pieceCost?scrapCost/pieceCost*100:0;
 return {produced,scrap,scrapRate:produced?scrap/produced*100:0,ppm:produced?scrap/produced*1e6:0,yieldRate:produced?(produced-scrap)/produced*100:100,copq,copqPercent,pieceCost,scrapCost};
}
