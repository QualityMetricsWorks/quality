...
export async function insertDowntimeEvents(runId,items){if(!items?.length)return [];const rows=check(await db.from('downtime_events').insert(items.map(x=>({...common(),run_id:runId,reason_id:x.reasonId,minutes:x.minutes,event_type:x.eventType||'unplanned',notes:[x.reason,x.notes].filter(Boolean).join(' · ')||null}))).select());state.downtimeEvents=[...rows.map(x=>({id:x.id,runId:x.run_id,reasonId:x.reason_id,minutes:Number(x.minutes||0),eventType:x.event_type||'unplanned',notes:x.notes||'',createdAt:x.created_at||new Date().toISOString()})),...state.downtimeEvents];return rows}
