...
function initEvents(){
 ...
 $('runClient').addEventListener('change',ui.updateRunParts);$('runPart').addEventListener('change',ui.updateRunOperations);$('defectClient').addEventListener('change',ui.updateDefectParts);$('defectPartNumber').addEventListener('change',ui.updateDefectOperations);$('scrapRun').addEventListener('change',ui.updateScrapDefects);
 $('productionRunForm').addEventListener('submit',e=>{e.preventDefault();if(!roleCanWrite())return toast('Tu rol es de solo lectura.');run(()=>api.insertRun({date:$('runDate').value,shift:$('runShift').value,clientId:$('runClient').value,partId:$('runPart').value,operationId:$('runOperation').value,machine:$('runMachine').value.trim(),produced:Number($('runProduced').value),plannedMinutes:Number($('runPlannedMinutes').value||0),notes:$('runNotes').value.trim()})).then(()=>{e.target.reset();$('runDate').value=today()})});
 ...
 document.body.addEventListener('click',e=>{const t=e.target.closest('[data-client-id],[data-part-id],[data-open-part],[data-delete-operation],[data-delete-defect],[data-delete-run],[data-delete-scrap]');if(!t)return;
