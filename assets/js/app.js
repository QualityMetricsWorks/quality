...
let db;
function setView(view){document.querySelectorAll('.view').forEach(x=>x.classList.remove('active'));$(`${view}View`)?.classList.add('active');document.querySelectorAll('.nav-item').forEach(x=>x.classList.toggle('active',x.dataset.view===view));$('exportBtn').hidden=view!=='history';$('pageTitle').textContent=({dashboard:'Dashboard',capture:'Captura',clients:'Clientes',parts:'Números de Parte',machines:'Máquinas',personnel:'Personal',catalog:'Catálogo',history:'Historial'})[view]||view}
async function reload(msg='Datos actualizados'){await api.loadAll();ui.renderAll();$('storageStatus').textContent='Sistema Actualizado';if(msg)toast(msg)}
...
function initEvents(){
 ...
 $('clearFiltersBtn').addEventListener('click',()=>{['filterStart','filterEnd','filterClient','filterPartNumber','filterMachine'].forEach(id=>$(id).value='');$('filterPeriod').value='current';ui.updateFilterParts();ui.renderDashboard()});
 ...
 document.querySelectorAll('.dashboard-tab[data-dashboard-tab]').forEach(b=>b.addEventListener('click',()=>{document.querySelectorAll('[data-dashboard-tab]').forEach(x=>x.classList.remove('active'));document.querySelectorAll('[data-dashboard-panel]').forEach(x=>x.classList.remove('active'));b.classList.add('active');document.querySelector(`[data-dashboard-panel="${b.dataset.dashboardTab}"]`).classList.add('active');document.querySelector('.maintenance-filter').hidden=b.dataset.dashboardTab!=='maintenance';ui.renderDashboard()}));
 document.querySelectorAll('[data-catalog-tab]').forEach(b=>b.addEventListener('click',()=>{document.querySelectorAll('[data-catalog-tab]').forEach(x=>x.classList.remove('active'));document.querySelectorAll('[data-catalog-panel]').forEach(x=>x.classList.remove('active'));b.classList.add('active');document.querySelector(`[data-catalog-panel="${b.dataset.catalogTab}"]`).classList.add('active')}));
 $('filterPeriod').addEventListener('change',()=>{applyPeriod($('filterPeriod').value);ui.renderDashboard()});$('filterMachine').addEventListener('change',ui.renderDashboard);
 document.querySelectorAll('.history-tab').forEach(b=>b.addEventListener('click',()=>{document.querySelectorAll('.dashboard-tab[data-dashboard-tab]').forEach(b=>b.addEventListener('click',()=>{document.querySelectorAll('[data-dashboard-tab]').forEach(x=>x.classList.remove('active'));document.querySelectorAll('[data-dashboard-panel]').forEach(x=>x.classList.remove('active'));b.classList.add('active');document.querySelector(`[data-dashboard-panel="${b.dataset.dashboardTab}"]`).classList.add('active');document.querySelector('.maintenance-filter').hidden=b.dataset.dashboardTab!=='maintenance';ui.renderDashboard()}));
 document.querySelectorAll('[data-catalog-tab]').forEach(b=>b.addEventListener('click',()=>{document.querySelectorAll('[data-catalog-tab]').forEach(x=>x.classList.remove('active'));document.querySelectorAll('[data-catalog-panel]').forEach(x=>x.classList.remove('active'));b.classList.add('active');document.querySelector(`[data-catalog-panel="${b.dataset.catalogTab}"]`).classList.add('active')}));
 $('filterPeriod').addEventListener('change',()=>{applyPeriod($('filterPeriod').value);ui.renderDashboard()});$('filterMachine').addEventListener('change',ui.renderDashboard);
 document.querySelectorAll('.history-tab').forEach(x=>x.classList.remove('active'));document.querySelectorAll('.history-panel').forEach(x=>x.classList.remove('active'));b.classList.add('active');document.querySelector(`[data-history-panel="${b.dataset.historyTab}"]`).classList.add('active')}));
...
$('editPartBtn').addEventListener('click',()=>{$('partEditForm').hidden=false});$('cancelPartEditBtn').addEventListener('click',()=>{$('partEditForm').hidden=true});
...
$('machineSearch').addEventListener('input',ui.renderMachines);
  ...
  $('downtimeReasonForm').addEventListener('submit',e=>{e.preventDefault();run(()=>api.insertDowntimeReason({code:$('downtimeCode').value.trim(),name:$('downtimeName').value.trim(),category:$('downtimeCategory').value})).then(()=>e.target.reset())});$('downtimeSearch').addEventListener('input',ui.renderDowntimeCatalog);
 $('cancelScrapBtn').addEventListener('click',()=>{ $('scrapMatchForm').reset();$('scrapEventForm').reset();$('scrapRun').value='';$('matchedRunsList').innerHTML='';$('selectedRunSummary').textContent='Completa los datos para localizar la corrida.';ui.renderSelects();toast('Captura cancelada')});
 ...
 document.body.addEventListener('click',e=>{const t=e.target.closest('[data-client-id],[data-part-id],[data-open-part],[data-machine-id],[data-personnel-id],[data-match-run],[data-unlink-machine],[data-delete-operation],[data-delete-defect],[data-delete-run],[data-delete-scrap],[data-delete-downtime-reason]');if(!t)return;
  ...
  if(t.dataset.deleteScrap&&confirm('¿Eliminar evento?'))run(()=>api.deleteScrapEvent(t.dataset.deleteScrap));if(t.dataset.deleteDowntimeReason&&confirm('¿Desactivar motivo de paro?'))run(()=>api.deleteDowntimeReason(t.dataset.deleteDowntimeReason));
 });
...
async function startSession(user){
 try{await api.loadIdentity(user);await api.loadAll();$('authOverlay').classList.add('hidden');$('companyContext').textContent=state.companyName;$('userEmail').textContent=user.email;$('storageStatus').textContent='Sistema Actualizado';ui.renderAll()}catch(e){console.error(e);toast(e.message);$('authOverlay').classList.remove('hidden')}
}
function iso(d){return d.toISOString().slice(0,10)}
function applyPeriod(v){const now=new Date(),s=new Date(now),e=new Date(now);if(v==='custom')return;if(v==='current'){s.setDate(1)}else if(v==='previous_day'){s.setDate(now.getDate()-1);e.setDate(now.getDate()-1)}else if(v==='previous_week'){const day=(now.getDay()+6)%7;e.setDate(now.getDate()-day-1);s.setTime(e.getTime());s.setDate(e.getDate()-6)}else if(v==='previous_month'){s.setMonth(now.getMonth()-1,1);e.setDate(0)}else if(v==='previous_quarter'){const q=Math.floor(now.getMonth()/3);s.setMonth((q-1)*3,1);e.setMonth(q*3,0)}else if(v==='previous_half'){const half=now.getMonth()<6?0:1;s.setFullYear(half?now.getFullYear():now.getFullYear()-1);s.setMonth(half?0:6,1);e.setFullYear(s.getFullYear());e.setMonth(s.getMonth()+6,0)}else if(v==='previous_year'){s.setFullYear(now.getFullYear()-1,0,1);e.setFullYear(now.getFullYear()-1,11,31)}$('filterStart').value=iso(s);$('filterEnd').value=iso(e)}
async function init(){
 db=api.initDb();initEvents();window.GUVEL_RENDER_BARCODE=renderBarcode;initTraceability(()=>reload('Producción registrada'));applyPeriod('current');const {data:{session}}=await db.auth.getSession();if(session?.user)await startSession(session.user);
