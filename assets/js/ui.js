...
export 
  ['scrapHistoryShift','downtimeHistoryShift'].forEach(id=>{const el=$(id);if(el){const v=el.value;el.innerHTML='<option value="">Todos los turnos</option>'+state.shiftSchedules.map(s=>`<option value="${esc(s.code)}">${esc(s.code)} · ${esc(s.name)}</option>`).join('');if([...el.options].some(o=>o.value===v))el.value=v;}});
