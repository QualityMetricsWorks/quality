...
export function renderSelects(){
 ...
 const shift=$('dataShiftFilter');if(shift){const current=shift.value;shift.innerHTML='<option value="">Todos los turnos</option>'+state.shiftSchedules.map(x=>`<option value="${esc(x.code)}">${esc(x.code)} · ${esc(x.name)}</option>`).join('');if([...shift.options].some(o=>o.value===current))shift.value=current;}
}
...
export function renderHistory(){
 ...
 const shift=($('dataShiftFilter')?.value||'').trim();
 const startDate=$('filterStart')?.value||'';const endDate=$('filterEnd')?.value||'';const clientId=$('filterClient')?.value||'';const partId=$('filterPartNumber')?.value||'';const machineId=$('filterMachine')?.value||'';
 ...
 const rs=[...state.runs].sort(newest).filter(r=>matchesRun(r)&&`${r.date} ${r.shift} ${r.lotNumber} ${getClient(r.clientId)?.name} ${getPart(r.partId)?.number} ${getOperation(r.operationId)?.code} ${r.machine} ${getPersonnel(r.operatorId)?.fullName}`.toLowerCase().includes(qp));
 ...
 const qs=($('scrapHistorySearch')?.value||'').toLowerCase();
 const es=[...state.scrapEvents].sort(newest).filter(e=>{const r=getRun(e.runId);return r&&matchesRun(r)&&`${r.date} ${r.shift} ${getClient(r.clientId)?.name} ${getPart(r.partId)?.number} ${getDefect(e.defectId)?.name} ${e.disposition} ${e.reason||''} ${e.notes||''}`.toLowerCase().includes(qs)});
 $('scrapHistoryBody').innerHTML=es.map(e=>{const r=getRun(e.runId),p=getPart(r?.partId);return `<tr><td>${r?.date||'—'}</td><td>${esc(r?.shift||'—')}</td><td>${esc(getClient(r?.clientId)?.name||'—')}</td><td>${esc(p?.number||'—')}</td><td>${esc(getOperation(r?.operationId)?.code||'—')}</td><td>${esc(getDefect(e.defectId)?.name||'—')}</td><td>${number(e.quantity)}</td><td>${esc(dispositionLabel(e.disposition))}</td><td>${esc(e.reason||'—')}</td><td>${esc(e.notes||'—')}</td><td>${money(copqForEvent(e),p?.currency||'USD')}</td><td><button class="icon-btn" data-delete-scrap="${e.id}">×</button></td></tr>`}).join('')||'<tr><td colspan="12" class="empty-state">Sin eventos.</td></tr>';
 const dh=$('downtimeHistoryBody');if(dh){const qd=($('downtimeHistorySearch')?.value||'').toLowerCase();const esd=[...state.downtimeEvents].sort(newest).filter(e=>{const r=getRun(e.runId),reason=getDowntimeReason(e.reasonId);return r&&matchesRun(r)&&`${r.date} ${r.shift} ${getClient(r.clientId)?.name} ${getPart(r.partId)?.number} ${getMachine(r.machineId)?.code} ${reason?.name} ${e.eventType} ${e.notes||''}`.toLowerCase().includes(qd)});dh.innerHTML=esd.map(e=>{const r=getRun(e.runId),reason=getDowntimeReason(e.reasonId);return `<tr><td>${r?.date||'—'}</td><td>${esc(r?.shift||'—')}</td><td>${esc(getClient(r?.clientId)?.name||'—')}</td><td>${esc(getPart(r?.partId)?.number||'—')}</td><td>${esc(getMachine(r?.machineId)?.code||'—')}</td><td>${esc(reason?.name||'—')}</td><td>${esc(e.eventType==='planned'?'Planned':'Unplanned')}</td><td>${esc(reason?.name||'—')}</td><td>${esc(e.notes||'—')}</td><td>${number(e.minutes)}</td><td><button class="icon-btn" data-delete-downtime="${e.id}">×</button></td></tr>`}).join('')||'<tr><td colspan="11" class="empty-state">Sin tiempos muertos.</td></tr>';}}
