...
function initEvents(){
 ...
 document.querySelectorAll('.capture-tab').forEach(b=>b.addEventListener('click',()=>{document.querySelectorAll('.capture-tab').forEach(x=>x.classList.remove('active'));document.querySelectorAll('.capture-panel').forEach(x=>x.classList.remove('active'));b.classList.add('active');$(`capture${b.dataset.captureTab[0].toUpperCase()+b.dataset.captureTab.slice(1)}Panel`).classList.add('active')}));
 ...
 $('runClient').addEventListener('change',ui.updateRunParts);$('runPart').addEventListener('change',ui.updateRunOperations);$('scrapClient').addEventListener('change',ui.updateScrapParts);$('scrapPart').addEventListener('change',ui.updateScrapOperations);$('scrapDate').addEventListener('change',ui.findMatchingRuns);$('scrapShift').addEventListener('change',ui.findMatchingRuns);$('scrapOperation').addEventListener('change',ui.findMatchingRuns);$('scrapMachine').addEventListener('change',ui.findMatchingRuns);$('defectClient').addEventListener('change',ui.updateDefectParts);$('defectPartNumber').addEventListener('change',ui.updateDefectOperations);$('findProductionBtn').addEventListener('click',ui.findMatchingRuns);
 $('productionRunForm').addEventListener('submit',e=>{e.preventDefault();if(!roleCanWrite())return toast('Tu rol es de solo lectura.');run(()=>api.insertRun({date:$('runDate').value,shift:$('runShift').value,clientId:$('runClient').value,partId:$('runPart').value,operationId:$('runOperation').value,machineId:$('runMachine').value,machine:'',produced:Number($('runProduced').value),plannedMinutes:Number($('runPlannedMinutes').value||0),notes:$('runNotes').value.trim()})).then(()=>{e.target.reset();$('runDate').value=today()})});
...
$('machineSearch').addEventListener('input',ui.renderMachines);
 ...
 document.body.addEventListener('click',e=>{const t=e.target.closest('[data-client-id],[data-part-id],[data-open-part],[data-machine-id],[data-match-run],[data-unlink-machine],[data-delete-operation],[data-delete-defect],[data-delete-run],[data-delete-scrap]');if(!t)return;
...
function exportExcel(){
 ...
 const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(production),'Produccion');XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(scrap),'Scrap');XLSX.writeFile(wb,'Quality_Summary_0.3.0.xlsx');
}
async function startSession(user){
 try{await api.loadIdentity(user);await api.loadAll();$('authOverlay').classList.add('hidden');$('companyContext').textContent=state.companyName;$('userEmail').textContent=user.email;$('storageStatus').textContent='Supabase sincronizado';ui.renderAll()}catch(e){console.error(e);toast(e.message);$('authOverlay').classList.remove('hidden')}
}
async function init(){
 db=api.initDb();initEvents();$('runDate').value=today();const {data:{session}}=await db.auth.getSession();if(session?.user)await startSession(session.user);
